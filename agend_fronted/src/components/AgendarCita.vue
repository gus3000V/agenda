<template>
  <div class="h-screen w-full bg-gray-50 flex flex-col font-sans overflow-hidden">
    ...
    <!-- Encabezado -->
    <header class="bg-blue-600 text-white px-6 py-4 shadow-md shrink-0 flex justify-between items-center z-10">
      <div>
        <h1 class="text-2xl font-extrabold tracking-tight">Clínica Médica</h1>
        <p class="text-sm text-blue-100 mt-1">Agenda tu cita seleccionando un horario libre</p>
      </div>
    </header>
    ...
    <main class="flex-1 overflow-hidden p-4">
      <div class="grid grid-cols-6 gap-3 h-full">
        <!-- Columnas por Día -->
        <div v-for="dia in diasSemana" :key="dia.fechaStr" class="bg-white flex flex-col rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          
          <!-- Encabezado del Día -->
          <div class="bg-gray-100 text-center py-3 shrink-0 border-b border-gray-200">
            <div class="font-bold text-gray-800 capitalize">{{ dia.nombreDia }}</div>
            <div class="text-sm text-gray-500 font-medium">{{ dia.fechaCorta }}</div>
          </div>

          <!-- Bloques Libres -->
          <div class="flex-1 overflow-y-auto p-3 space-y-2 relative">
            <div v-if="dia.cargando && dia.disponibles.length === 0" class="flex justify-center items-center h-full text-gray-400">
            ...
            <template v-else>
              <button 
                ...
                :class="[
                  'w-full py-2 rounded-md text-sm font-semibold transition-all shadow-sm flex flex-col items-center',
                  slot.estado === 'libre' 
              ...
              >
                <span>{{ slot.hora }}</span>
                <span v-if="slot.estado === 'pendiente'" class="text-[10px] uppercase font-bold mt-0.5">En espera</span>
              </button>
              
              <div v-if="dia.disponibles.length === 0" class="flex flex-col items-center justify-center h-full text-gray-400">
                <span class="text-3xl mb-2">🗓️</span>
                <span class="text-sm font-medium text-center">Sin citas<br>disponibles</span>
              </div>
            </template>
          </div>
        </div>
    ...
    <div v-if="modalAbierto" class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex justify-end z-40 transition-opacity">
      <div class="w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-slide-in">
        ...
        <div class="px-6 py-4 bg-white border-b border-gray-200 flex justify-between items-center shrink-0">
          <h2 class="text-xl font-bold text-gray-800">Completar Solicitud</h2>
        ...
        <div class="p-6 flex-1 overflow-y-auto bg-gray-50">
          ...
          <div class="mb-6 p-4 bg-blue-50 border border-blue-100 rounded-lg shadow-sm">
            <p class="text-sm text-blue-800">Estás agendando cita para el:</p>
          ...
          <form @submit.prevent="enviarSolicitud" class="space-y-5">
            <div>
              ...
              <input type="text" v-model="form.nombre" required placeholder="Ej: Juan Pérez"
                     class="w-full border border-gray-300 p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none shadow-sm" />
            </div>
    ...
    <div v-if="successMsg" class="fixed bottom-10 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-6 py-4 rounded-full shadow-2xl z-50 flex items-center font-medium animate-fade-in-up">
      <span class="bg-green-500 rounded-full w-6 h-6 flex items-center justify-center mr-3">✓</span>
      {{ successMsg }}
    </div>
